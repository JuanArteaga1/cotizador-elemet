...
### 3.3 Planes de Pagos (Auth: Customer) — NUEVO

El arquitecto crea planes de pagos reutilizables y los asigna a cotizaciones.

| Uso en Cotizador | Endpoint SaaS | Metodo | Body |
|---|---|---|---|
| Listar mis planes | `/api/v1/payment-plans` | GET | — |
| Crear plan | `/api/v1/payment-plans` | POST | `{ name, description, installments, isDefault }` |
| Ver plan | `/api/v1/payment-plans/:id` | GET | — |
| Editar plan | `/api/v1/payment-plans/:id` | PUT | `{ name, description, installments, isDefault }` |
| Eliminar plan | `/api/v1/payment-plans/:id` | DELETE | — |
| Marcar como default | `/api/v1/payment-plans/:id/default` | PATCH | — |

**Estructura de `installments`:**
```json
[
  { "name": "Firma", "percentage": 50, "order": 1 },
  { "name": "Revisión 1", "percentage": 30, "order": 2 },
  { "name": "Entrega", "percentage": 20, "order": 3 }
]
```

**Validacion:** la suma de `percentage` debe ser 100% (tolerancia ±0.1%).

### 3.4 Pagos Manuales de Cotizaciones (Auth: Customer) — NUEVO

Registro de pagos recibidos por cada cuota de una cotización. **No es MercadoPago**; es seguimiento manual de cobros.

| Uso en Cotizador | Endpoint SaaS | Metodo | Body |
|---|---|---|---|
| Listar pagos de cotización | `/api/v1/quotes/:id/payments` | GET | — |
| Registrar pago de cuota | `/api/v1/quotes/:id/payments` | POST | `{ installmentIndex, amount, method, notes, status }` |
| Revertir pago | `/api/v1/quotes/:id/payments/:paymentId` | DELETE | — |
...
**Reglas:**
- Una cuota (`installmentIndex`) solo puede tener **un** pago `confirmed` a la vez.
- Si ya existe, devolver `409 Conflict`.
- Al eliminar un pago, se marca como disponible nuevamente para registrar.

### 3.5 Configuracion de Precios (Auth: Customer)
